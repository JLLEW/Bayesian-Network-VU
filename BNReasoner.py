...
class BNReasoner:
    ...
    def min_fill_ordering(self, X: list):
        ...
        print(f"min fill degrees {min_fill_degrees}")
        min_fill_degrees.sort(key=lambda x: x[1])
        ordered = [x[0] for x in min_fill_degrees]
            
        return ordered

    def variable_elimination(self, X: list, ordering="min_degree") -> pd.DataFrame:
        if ordering == "min_degree":
            X = self.min_degree_ordering(X)
        else:
            X = self.min_fill_ordering(X)

        all_cpts = self.bn.get_all_cpts().values()
        
        prev_factor = None
        for x in X:
            x_cpts = [cpt for cpt in all_cpts if x in cpt.columns]
            all_cpts = [cpt for cpt in all_cpts if x not in cpt.columns]
            # add prev_factor
            x_cpts += [prev_factor]
